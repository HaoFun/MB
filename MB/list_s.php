...
$query="select * from fm_s_module WHERE  id={$_GET['id']}";//查詢fm_s_module是否有$_GET傳來的ID
...
$query="select * from fm_f_module WHERE id={$data_s['f_module_id']}";//查詢fm_f_module中f_module_id 是否等於子版內的f_module_id
$result_f=execute($link,$query);
$data_f=mysqli_fetch_assoc($result_f);

$query="select count(*) from fm_content WHERE module_id={$_GET['id']}";//查詢總帖子數量
$count_all=num($link,$query);

$query="select count(*) from fm_content WHERE module_id={$_GET['id']} and time>CURDATE()";//查詢今日新增的數量
$count_today=num($link,$query);

$query="select * from fm_member WHERE id={$data_s['member_id']}";  //查詢是否有版主
$result_member=execute($link,$query);
$title_name['title']="Gentry鞋靴櫃_".$data_s['module_name'];
?>
...
<div id="position" class="auto">
    <a href="index.php">首頁</a> &gt; <a href="list_f.php?id=<?PHP echo $data_f['id']; ?>"><?PHP echo $data_f['module_name']; ?></a> &gt; <a href="list_s.php?id=<?PHP echo $data_s['id']; ?>"><?PHP echo $data_s['module_name']; ?></a>
</div>
<div id="main" class="auto">
    <div id="left">
        ...
        <div class="pages_wrap">
            ...
            ?>
            <button class="publish" onclick="javascript:location.href='publish.php?s_module_id=<?PHP echo $_GET['id']?>'">發文</button>
            <div style="clear:both;"></div>
        </div>
    </div>
    <div id="right">
